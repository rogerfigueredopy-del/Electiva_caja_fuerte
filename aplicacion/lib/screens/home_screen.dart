...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Future<void> _load({bool force = false}) async {
    if (_busy && !force) {
      print('🔄 [LOAD] Carga ya en progreso, esperando...');
      return; // Evitar múltiples cargas simultáneas
    }
    
    print('🔄 [LOAD] Iniciando carga de archivos... (force: $force)');
    ...
    try {
      ...
      // Imprimir detalles de los archivos para debugging
      for (int i = 0; i < all.length && i < 5; i++) {
        print('🔄 [LOAD] Archivo $i: ${all[i].originalName} (${all[i].id})');
      }
      
      if (mounted) {
        setState(() { 
          _items = List.from(all); // Crear nueva lista para forzar rebuild
        ...
        // Forzar un rebuild adicional después de un breve delay
        if (force) {
          Future.delayed(const Duration(milliseconds: 50), () {
            if (mounted) {
              ...
              print('🔄 [LOAD] ✅ Rebuild forzado completado');
            }
          });
        }
      } else {
  ...
  Future<void> _import() async {
    ...
    try {
      ...
      for (final f in result.files) {
        ...
        print('📥 [IMPORT] Importando archivo: ${file.path}');
        ...
        importedCount++;
        print('📥 [IMPORT] ✅ Archivo importado exitosamente: ${file.path}');
      }
      
      print('📥 [IMPORT] ✅ Importación completada ($importedCount/$totalFiles), recargando lista...');
      
      // Forzar actualización de la lista con múltiples intentos
      await _load(force: true);
      print('📥 [IMPORT] ✅ Primera recarga completada, archivos actuales: ${_items.length}');
      
      // Pequeña pausa y segunda recarga para asegurar consistencia
      await Future.delayed(const Duration(milliseconds: 200));
      await _load(force: true);
      print('📥 [IMPORT] ✅ Segunda recarga completada, archivos actuales: ${_items.length}');
      ...
      if (mounted) {
        ...
        print('📥 [IMPORT] ✅ Estado del widget actualizado forzadamente');
      }
